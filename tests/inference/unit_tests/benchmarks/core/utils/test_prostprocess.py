...
def test_clip_boxes_coordinates(benchmark, num_predictions, origin_shape):
    predicted_bboxes = np.random.rand(num_predictions, 4)
    predicted_bboxes[:, 0] = (predicted_bboxes[:, 0] * 1.2 - 0.1) * origin_shape[
        1
    ]
    predicted_bboxes[:, 1] = (predicted_bboxes[:, 1] * 1.2 - 0.1) * origin_shape[
        0
    ]
    predicted_bboxes[:, 2] = (predicted_bboxes[:, 2] * 1.2 - 0.1) * origin_shape[
        1
    ]
    predicted_bboxes[:, 3] = (predicted_bboxes[:, 3] * 1.2 - 0.1) * origin_shape[
        0
    ]
    predicted_bboxes[:, 2] = np.maximum(
        predicted_bboxes[:, 0] + 1, predicted_bboxes[:, 2]
...
def test_post_process_keypoints(
    ...
    for kp_idx in range(num_keypoints):
        ...
        predictions_np[..., keypoints_start_index + kp_idx * 3 + 1] *= infer_shape[
            0
        ]
