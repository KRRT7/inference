...
@pytest.mark.parametrize(
    "input_type, input_value_getter",
    [
        ("b64_string", lambda images: images[0]),
        ("image_bytes", lambda images: images[1]),
        ("image_IO_buffer", lambda images: images[2]),
        # ("numpy_image_str", lambda images: images[3]),  # Uncomment when implemented
    ],
)
def test_prepare_image_to_registration(
    benchmark, images, input_type, input_value_getter
):
    input_value = input_value_getter(images)
    benchmark(
        attempt_loading_image_from_string,
        input_value,
    )
...
@pytest.mark.parametrize("jpeg_quality", [50, 75, 90, 95])
def test_encode_image_to_jpeg_bytes(benchmark, numpy_image, jpeg_quality):
    benchmark(
        encode_image_to_jpeg_bytes,
        numpy_image,
